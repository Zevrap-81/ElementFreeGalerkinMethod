...
#             print(GI)                                                # Global index number
            ...
            ycord[:,j]=xc[1,GI]                             # y-cord of global index
        for i in range(l):
            for j in range(l):
                eta1=gauss[0,i]
                eta2=gauss[0,j]
                N=0.25*(one+eta2*sign1)*(one+eta1*sign2)        # shape function in 2D 
    #             print(N)
                dNeta2=0.25*sign1*(one+eta1*sign2)
                #print(dNeta1.shape)                                       # derivative of shape function w.r.t x-axis transformed(eta1)
                dNeta1=0.25*sign2*(one+eta2*sign1)                # derivative of shape function w.r.t y-axis transformed(eta2)
    ...
    #             print(yeta1)
                ...
                gs[0,index] = xq                              # gauus point for x-cord
                gs[1,index] = yq                              # gauss point for y-cord
